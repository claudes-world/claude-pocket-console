  ...
  return (
    <BottomSheet onClose={props.onClose} title="File Options">
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <div style={{ fontSize: 12, color: "var(--color-muted)", marginTop: 4, marginBottom: 2 }}>Sort by:</div>
        {SORT_OPTIONS.map((opt) => (
          <button
            key={opt.value}
            onClick={() => { props.setFileSortMode?.(opt.value); props.onClose(); }}
            style={{ ...btnStyle, padding: "10px 14px", textAlign: "left", ...(props.fileSortMode === opt.value ? { background: "#2d3a5a", color: "var(--color-accent-blue)", border: "1px solid #3d4a6a" } : {}) }}
          >
            {opt.long}
